'''
* 크롤링 Crawling *
    크롤러 : 알고리즘에 의해 인터넷을 탐색하는 프로그램. 스크래퍼, 봇, 스파이더, 지능 에이전트라고도 함.
    크롤링 : 크롤러가 데이터를 수집하는 행위. 스크래핑

    # 웹
    - 클라이언트 : 서비스를 요청하는 프로그램
    - 서버 : 서비스를 제공해주는 프로그램
        * 종류: 상제공, 파일, 도메인, 채팅 및 음성, 게임, 전자우편, 웹 서버...

    # 헤더
    - 서버와 클라이언트 간에 데이터를 주고 받을 때, 헤더라는 정보를 포함해서 보낸다.

    요청 헤더   : 클라이언트 -> 서버 요청할 때, 정보 포함한 헤더
                 User-Agent(브라우저 정보), Method(요청 메서드), refered(이전에 머물던 주소)

    응답 헤더   : 서버 -> 클라이언트 흥답해 줄 때, 정보 포함한 헤더
                 Status-code(응답 코드)

    일반 헤더   : 양측 모두에서 사용되는 일반 정보 포함한 헤더
    엔티티     : 메세지에 해당하는 정보를 포함한 헤더 -> Content-Type(entity-body의 미디어 타입)
...
res = requests.get(url) # 요청하고 응답 객체 리턴해줌.
# print(res)
...
# html 코드
# print(res.text)
# print(res.content) # 한글을 바이너리 형태로 가져옴. -> text로 가져올 때, 한글이 깨지는 경우 content
# print(res.encoding)
...
print(res.url)
#. POST 요청 params : 쿼리스트링, body, data 문자 보내기
res = requests.post(url, data={'key1': 'value1', 'key2': 'value2'})
print(res.url)
# 간혹 정상적인 요청이 안 될 경우가 있다.
...
#. requests 오류 처리 방법
    ...
    URLRequired
    TooManyRedirects
    MissingSchema   : HTTP 또는 HTTPS 를 생략했을때
    InvalidURL
...
try:
    ...
    print("MissingSchema 에러. url에 http 프로토콜 확인.")
...
print(title['class'])
print(title['id'])
# print(title['name']) # 없는 속성을 꺼내면 에러 발생!
...
print(p_tag)
# print(p_tag.text)
# print(p_tag.string) # p 태그가 온전히 가지고 있는  text 가 없어서 None

children = soup.p.contents
# print(children)
children = soup.p.childern # 이터레이터로 리턴 -> 반복문으로 꺼내서 사용
# print(children)
